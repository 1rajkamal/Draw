# 🎨 Drawing Duel — Real-Time 2-Player Cartoon Drawing Game

A 2-player real-time web application where two people join a room via a 6-character invite code or QR code, select a difficulty tier, choose one of 10 reference cartoon images (Oggy, Chhota Bheem, Owl, Parrot, Baby, Flower, Pikachu, Doraemon, Shinchan & more), and race to recreate it on their canvas using drawing tools. Once both submit, an automated computer vision accuracy scoring engine evaluates both drawings and crowns the winner!

---

## ✨ Features

- **⚡ Real-Time Multiplayer Sync**: Powered by WebRTC P2P DataChannels & Socket.IO. Sub-10ms event synchronization for lobby, difficulty selection, live drawing status, and instant round results.
...
- **🖼️ 30 Built-in Cartoon Reference Artworks**:
  - **Easy (10 artworks)**: Smiling Flower, Cute Owl, Mithu the Parrot, Cute Small Baby, Peppa Pig, Pikachu Face, Hello Kitty, Winnie the Pooh, Tweety Bird, Baby Panda.
  - **Medium (10 artworks)**: Oggy the Cat, Chhota Bheem, Doraemon, Shin-chan, SpongeBob, Mickey Mouse, Motu with Samosa, Minion, Jerry Mouse, Tom Cat.
  - **Hard (10 artworks)**: Bheem at Dholakpur Palace, Oggy vs Cockroaches, Doraemon & Nobita Sky Flight, Baby Krishna Makhan Chor, Goku Super Saiyan, Pikachu & Ash, Spider-Man Web Sling, Motu & Patlu Chai Stall, Simba on Pride Rock, Shinchan as Action Kamen.
...
- **⚡ Floating Emoji Reactions**: Real-time floating reactions (`🔥`, `🎉`, `😂`, `👑`, `🎨`, `👀`) between players.
...
- **Frontend**: React 18, Vite, TypeScript, Tailwind CSS, Lucide Icons, Canvas-Confetti, QRCode, PeerJS
- **Canvas Engine**: HTML5 2D Context + Pointer Events API + Bézier Curve Interpolation
- **Backend & Networking**: WebRTC DataChannels + Node.js/Express + Socket.IO
...
2. **Player 2**: Enter your name and either scan the QR code or enter the 6-character room code.
3. **Lobby**: Host selects difficulty (Easy / Medium / Hard) and round timer (60s, 90s, 120s, or Untimed).
4. **Drawing Round**: Recreate the cartoon prompt before the timer expires!
